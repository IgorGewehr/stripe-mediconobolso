...
// ==============================================
    ...
    async createSecretaryAccount(doctorId, secretaryData) {
        ...
        try {
            ...
            // ✅ STEP 3: VERIFICAR SE EMAIL JÁ EXISTE
            ...
            // ✅ STEP 9: DELETAR INSTÂNCIA TEMPORÁRIA
    ...
    async checkEmailExistsInSystem(email) {
        ...
        try {
            ...
            // ✅ VERIFICAR NO FIREBASE AUTH USANDO INSTÂNCIA TEMPORÁRIA
            try {
                const { initializeApp } = await import('firebase/app');
                ...
                const tempAuth = getAuth(tempApp);
                const signInMethods = await fetchSignInMethodsForEmail(tempAuth, email);

                await tempApp.delete();
                tempApp = null;

                if (signInMethods.length > 0) {
                    return { exists: true, type: 'autenticação' };
                }
            } catch (authError) {
                if (tempApp) {
                    await tempApp.delete();
                ...
                console.warn('⚠️ Erro ao verificar Auth (não crítico):', authError);
            }
            ...
            if (tempApp) {
                try {
                    ...
                    console.error('❌ Erro no cleanup de verificação:', cleanupError);
                }
            }
    ...
    /**
     * ✅ BUSCAR INFORMAÇÕES DA SECRETÁRIA DO MÉDICO
     */
    ...
    /**
     * ✅ LISTAR TODAS AS SECRETÁRIAS DO MÉDICO
     */
    ...
    async reactivateSecretaryAccount(doctorId, secretaryId) {
        try {
            ...
            const validation = await this.validateSecretaryCreation(doctorId, { name: 'temp', email: 'temp@temp.com', password: 'temp123' });
