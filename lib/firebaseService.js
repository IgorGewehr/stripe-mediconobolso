...
class FirebaseService {
    ...
    async validateSecretaryCreation(doctorId, secretaryData) {
        console.log(`🔍 Validando criação de secretária para médico: ${doctorId}`);

        // Validações básicas
        if (!doctorId || !secretaryData?.email || !secretaryData?.password || !secretaryData?.name) {
            throw new Error("Dados obrigatórios: doctorId, email, password, name");
        }

        // Verificar se médico está logado
        const currentUser = this.auth.currentUser;
        if (!currentUser || currentUser.uid !== doctorId) {
            throw new Error("Médico deve estar logado para criar secretária");
        }
        ...
        // ✅ REMOÇÃO DA RESTRIÇÃO DE PLANO - TODOS OS MÉDICOS PODEM TER SECRETÁRIAS
        console.log(`✅ Médico ${doctorData.fullName} autorizado a criar secretária (plano: ${doctorData.planType || 'gratuito'})`);

        // ✅ VERIFICAR LIMITE DE SECRETÁRIAS BASEADO NO PLANO
        const secretaryCount = await this.getDoctorSecretaryCount(doctorId);
        ...
        if (doctorData.administrador) {
            ...
            MAX_SECRETARIES = 1; // ✅ Plano gratuito pode ter 1 secretária
            planName = 'Gratuito';
        }

        console.log(`📊 Limite para plano ${planName}: ${secretaryCount}/${MAX_SECRETARIES} secretárias`);

        if (secretaryCount >= MAX_SECRETARIES) {
            const upgradeMessage = !doctorData.assinouPlano && !doctorData.administrador ?
                ' Faça upgrade para ter mais secretárias.' : '';
            throw new Error(`Limite máximo de secretárias atingido para plano ${planName} (${MAX_SECRETARIES}).${upgradeMessage}`);
        }
        ...
        if (emailCheck.exists) {
            const errorMessages = {
                'doctor': "Este email já está sendo usado por um médico",
                'secretary': "Este email já está sendo usado por outra secretária"
            };
            throw new Error(errorMessages[emailCheck.type] || "Este email já está em uso no sistema");
        }

        // Validar senha
        if (secretaryData.password.length < 8) {
        ...
        return {
            ...
            MAX_SECRETARIES,
            planName,
            canCreateMore: secretaryCount < MAX_SECRETARIES
        };
    }

// ✅ NOVA FUNÇÃO PARA OBTER INFORMAÇÕES DE LIMITE POR PLANO
    async getDoctorSecretaryLimits(doctorId) {
        try {
            const doctorData = await this.getUserData(doctorId);
            if (!doctorData) {
                throw new Error("Médico não encontrado");
            ...
            return {
                current: secretaryCount,
                max: maxSecretaries,
                planName: planName,
                canCreateMore: secretaryCount < maxSecretaries,
                remaining: Math.max(0, maxSecretaries - secretaryCount)
            };
        } catch (error) {
            console.error("❌ Erro ao obter limites de secretária:", error);
            return {
                current: 0,
                max: 1,
                planName: 'Gratuito',
    ...
    /**
     * ✅ STEP 2: Salvar sessão do médico
     */
    saveDoctorSession(currentUser) {
        ...
        console.log(`💾 Salvando sessão do médico: ${doctorSessionData.email}`);
        localStorage.setItem('tempDoctorSession', JSON.stringify(doctorSessionData));

        return doctorSessionData;
    }

    /**
     * ✅ STEP 3: Criar conta Firebase Auth
     */
    async createSecretaryFirebaseAuth(secretaryData) {
        console.log(`👩‍💼 Criando conta Firebase Auth para secretária...`);
        ...
        try {
            ...
            console.log(`✅ Conta Firebase Auth criada: ${secretaryId}`);
            console.log(`⚠️ Firebase fez login automático com a secretária`);

            return secretaryId;
        } catch (error) {
            // Tratar erros específicos do Firebase Auth
            const errorMessages = {
                'auth/email-already-in-use': "Este email já está registrado no Firebase Auth",
                'auth/weak-password': "Senha muito fraca. Use pelo menos 8 caracteres.",
                'auth/invalid-email': "Email inválido"
            };

            throw new Error(errorMessages[error.code] || `Erro ao criar conta: ${error.message}`);
        }
    }

    /**
     * ✅ STEP 4: Criar documento da secretária
     */
    async createSecretaryDocument(secretaryId, doctorId, secretaryData) {
        console.log(`📄 Criando documento da secretária: ${secretaryId}`);
        ...
        const secretaryDocData = {
            ...
            active: true,
            permissions: secretaryData.permissions || defaultPermissions,
            createdAt: new Date(),
            createdBy: doctorId,
            lastLogin: null,
            loginCount: 0,
            version: "1.2",
            userAgentCreation: typeof navigator !== 'undefined' ? navigator.userAgent : '',
        ...
        try {
            await runTransaction(this.firestore, async (transaction) => {
                ...
                transaction.set(secretaryRef, secretaryDocData);
            });

            console.log(`✅ Documento secretária criado: secretaries/${secretaryId}`);
            return secretaryDocData;
        } catch (error) {
            console.error(`❌ Erro ao criar documento da secretária: ${error}`);
            throw new Error("Erro ao salvar dados da secretária no banco");
        }
    }

    /**
     * ✅ STEP 5: Atualizar configuração do médico
     */
    async verifySecretaryCreation(doctorId, secretaryId) {
        try {
            console.log(`🔍 Verificando criação da secretária ${secretaryId}...`);

            // Aguardar um pouco para propagação dos dados
            await new Promise(resolve => setTimeout(resolve, 1000));

            // Verificar se documento da secretária existe
            const secretaryRef = doc(this.firestore, "secretaries", secretaryId);
            const secretarySnap = await getDoc(secretaryRef);

            if (!secretarySnap.exists()) {
                throw new Error('Documento da secretária não foi criado');
            }

            const secretaryData = secretarySnap.data();
            if (!secretaryData.active || secretaryData.doctorId !== doctorId) {
                throw new Error('Dados da secretária estão incorretos');
            }

            // Verificar se configuração do médico foi atualizada
            const doctorData = await this.getUserData(doctorId);
            if (!doctorData.secretaryConfig?.hasSecretary ||
                doctorData.secretaryConfig.secretaryId !== secretaryId) {
                throw new Error('Configuração do médico não foi atualizada');
            }

            console.log(`✅ Verificação bem-sucedida: secretária ${secretaryData.name} criada para Dr. ${doctorData.fullName}`);
            return true;
        } catch (error) {
            console.error('❌ Falha na verificação da criação:', error);
            throw new Error(`Verificação falhou: ${error.message}`);
        }
    }

// 5. ✅ FUNÇÃO MELHORADA PARA ATUALIZAR CONFIGURAÇÃO DO MÉDICO
    async updateDoctorConfiguration(doctorId, secretaryId, secretaryData, secretaryCount) {
        console.log(`⚙️ Atualizando configuração do médico: ${doctorId}`);

        try {
            const doctorRef = doc(this.firestore, "users", doctorId);

            // ✅ CONFIGURAÇÃO MAIS ROBUSTA E DETALHADA
            ...
            console.log(`✅ Configuração do médico atualizada com secretaryId: ${secretaryId}`);

            // ✅ AGUARDAR E VERIFICAR SE OS DADOS FORAM SALVOS
            await new Promise(resolve => setTimeout(resolve, 500));

            const updatedDoc = await getDoc(doctorRef);
            const savedData = updatedDoc.data();

            if (savedData.secretaryConfig?.secretaryId === secretaryId) {
                console.log(`✅ Verificação: Dados salvos corretamente`);
            } else {
                console.error(`❌ Verificação: Dados não foram salvos corretamente`, savedData.secretaryConfig);
                throw new Error("Falha na verificação dos dados salvos");
            }

        } catch (error) {
            console.error(`❌ Erro ao atualizar configuração do médico: ${error}`);
            throw new Error("Erro ao atualizar configuração do médico");
        }
    }

// 6. ✅ FUNÇÃO MELHORADA PARA RESTAURAR SESSÃO DO MÉDICO
    async restoreDoctorSession(doctorEmail, password) {
        try {
            console.log(`🔐 Restaurando sessão do médico: ${doctorEmail}`);

            if (!doctorEmail || !password) {
                throw new Error("Email e senha são obrigatórios para restaurar sessão");
            }

            // Fazer login do médico novamente
            const result = await signInWithEmailAndPassword(this.auth, doctorEmail, password);

            if (!result.user) {
                throw new Error("Falha na autenticação do médico");
            }

            console.log(`✅ Sessão do médico restaurada com sucesso: ${result.user.uid}`);
            ...
            // ✅ VERIFICAR SE OS DADOS DA SECRETÁRIA ESTÃO ACESSÍVEIS
            const secretaryInfo = await this.getDoctorSecretaryInfo(result.user.uid);
            if (secretaryInfo) {
                console.log(`✅ Secretária ${secretaryInfo.name} está acessível após re-login`);
            }
            ...
            console.error("❌ Erro ao restaurar sessão do médico:", error);
    ...
    /**
     * ✅ STEP 6: Fazer logout da secretária
     */
    async logoutSecretaryAndPrepareResponse(doctorSessionData, secretaryId, secretaryData, doctorData, secretaryCount) {
        console.log(`🚪 Fazendo logout da secretária para restaurar sessão do médico...`);

        try {
            await signOut(this.auth);
            console.log(`✅ Logout da secretária realizado com sucesso`);
        } catch (error) {
            console.warn(`⚠️ Erro no logout da secretária (não crítico): ${error}`);
        }
    ...
    /**
     * ✅ STEP 7: Cleanup em caso de erro
     */
    async cleanupOnError(secretaryId, error) {
        console.log(`🧹 Executando cleanup devido ao erro: ${error.message}`);

        try {
            // Remover dados temporários
            localStorage.removeItem('tempDoctorSession');

            // Se criou a conta mas deu erro depois, tentar deletar
            const currentUser = this.auth.currentUser;
            if (currentUser && currentUser.uid === secretaryId) {
                console.log('🔄 Tentando deletar conta criada...');
                await currentUser.delete();
                console.log('✅ Conta deletada com sucesso');
            }

            // Tentar remover documento da secretária se foi criado
            try {
                const secretaryRef = doc(this.firestore, "secretaries", secretaryId);
                await deleteDoc(secretaryRef);
                console.log('✅ Documento da secretária removido');
            } catch (docError) {
                console.warn('⚠️ Erro ao remover documento da secretária:', docError);
            }
    ...
    async createSecretaryAccount(doctorId, secretaryData) {
        ...
        let doctorSessionData = null;

        try {
            console.log(`🚀 Iniciando processo de criação de secretária...`);

            // STEP 1: Validações
            const { doctorData, secretaryCount } = await this.validateSecretaryCreation(doctorId, secretaryData);

            // STEP 2: Salvar sessão do médico
            doctorSessionData = this.saveDoctorSession(this.auth.currentUser);

            // STEP 3: Criar conta Firebase Auth
            secretaryId = await this.createSecretaryFirebaseAuth(secretaryData);

            // STEP 4: Criar documento da secretária
            await this.createSecretaryDocument(secretaryId, doctorId, secretaryData);

            // STEP 5: Atualizar configuração do médico
            await this.updateDoctorConfiguration(doctorId, secretaryId, secretaryData, secretaryCount);

            // ✅ STEP 5.5: VERIFICAR SE OS DADOS FORAM SALVOS CORRETAMENTE
            await this.verifySecretaryCreation(doctorId, secretaryId);

            // STEP 6: Logout e preparar resposta
            const result = await this.logoutSecretaryAndPrepareResponse(
                doctorSessionData,
                secretaryId,
                secretaryData,
                doctorData,
            ...
            if (secretaryId) {
                await this.cleanupOnError(secretaryId, error);
            }
    ...
    /**
     * ✅ FUNÇÃO AUXILIAR PARA MONITORAR TRANSAÇÕES
     */
    async runTransactionWithRetry(transactionFn, maxRetries = 3) {
        let lastError;

        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                console.log(`🔄 Tentativa ${attempt}/${maxRetries} de transação...`);
                const result = await runTransaction(this.firestore, transactionFn);
                console.log(`✅ Transação realizada com sucesso na tentativa ${attempt}`);
                return result;
            } catch (error) {
                ...
                if (attempt < maxRetries) {
                    // Aguardar um tempo exponencial antes da próxima tentativa
    ...
    async checkEmailExistsInSystem(email) {
        try {
            // Verificar na collection users (médicos)
            const usersRef = collection(this.firestore, "users");
            ...
            // Verificar na collection secretaries
            const secretariesRef = collection(this.firestore, "secretaries");
            const secretaryQuery = query(secretariesRef, where("email", "==", email), where("active", "==", true));
            ...
            return { exists: true }; // Em caso de erro, assumir que existe (mais seguro)
        }
    }

    async getDoctorSecretaryCount(doctorId) {
        try {
            const secretariesRef = collection(this.firestore, "secretaries");
            const q = query(
                secretariesRef,
    ...
    /**
     * Verificar se email de secretária já existe
     * @param {string} email
     * @returns {Promise<boolean>}
     */
    async checkSecretaryEmailExists(email) {
        try {
            const secretariesRef = collection(this.firestore, "secretaries");
            const q = query(secretariesRef, where("email", "==", email), where("active", "==", true));
            const querySnapshot = await getDocs(q);
    ...
    /**
     * Buscar contexto unificado do usuário (OTIMIZADO)
     * @param {string} userId - ID do usuário logado
     * @returns {Promise<Object>} - Contexto unificado
     */
    async getUserUnifiedContext(userId) {
        try {
            console.log(`🔍 Verificando contexto unificado para: ${userId}`);


            // OTIMIZAÇÃO: Verificar se é secretária PRIMEIRO (mais provável em produção)
            try {
            ...
            try {
                ...
                console.error("❌ Usuário não é médico nem secretária:", doctorError);
    ...
    /**
     * Verificar permissão específica de módulo (COM CACHE)
     * @param {string} userId - ID do usuário
     * @param {string} module - Nome do módulo
     * @param {string} action - Ação (read/write/viewDetails)
     * @returns {Promise<boolean>}
     */
    async checkModulePermission(userId, module, action = 'read') {
        try {
            // Buscar contexto (usa cache automaticamente)
            const context = await this.getUserUnifiedContext(userId);

            // Médico sempre tem acesso total
            if (context.userType === 'doctor') {
                return true;
            }

            // Verificar permissões da secretária
            const modulePermissions = context.permissions[module];
     ...
     * @param {string} secretaryId - ID da secretária
     * @param {Object} newPermissions - Novas permissões
     */
    async updateSecretaryPermissions(doctorId, secretaryId, newPermissions) {
        try {
            ...
            if (secretaryData.doctorId !== doctorId) {
                throw new Error("Não autorizado a modificar esta secretária");
            }

            // Validar estrutura das permissões
            const validModules = ['patients', 'appointments', 'prescriptions', 'exams', 'notes', 'financial', 'reports', 'admin'];
            const validActions = ['read', 'write', 'viewDetails'];
            ...
            console.log(`✅ Permissões atualizadas para secretária: ${secretaryId}`);
            return true;
    ...
    async deactivateSecretaryAccount(doctorId, secretaryId) {
        try {
            console.log(`🔄 Desativando conta da secretária: ${secretaryId}`);
            ...
            console.log(`✅ Conta da secretária desativada: ${secretaryId}`);
            return true;
    ...
    /**
     * Buscar informações da secretária ativa do médico
     * @param {string} doctorId - ID do médico
    ...
    async getDoctorSecretaryInfo(doctorId) {
        try {
            ...
            console.log(`🔍 Buscando informações da secretária para médico: ${doctorId}`);

            // ✅ BUSCAR DADOS DO MÉDICO COM TIMEOUT
            const doctorDataPromise = this.getUserData(doctorId);
            const timeoutPromise = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Timeout')), 10000)
            );

            const doctorData = await Promise.race([doctorDataPromise, timeoutPromise]);

            if (!doctorData) {
                console.log('getDoctorSecretaryInfo: Dados do médico não encontrados');
                return null;
            }

            console.log('getDoctorSecretaryInfo: secretaryConfig encontrado:', doctorData.secretaryConfig);

            // ✅ VERIFICAR MÚLTIPLAS FONTES DE DADOS DA SECRETÁRIA
            let secretaryId = null;
            let secretaryInfo = null;

            // 1. Tentar obter da configuração atual
            if (doctorData.secretaryConfig?.secretaryId) {
                secretaryId = doctorData.secretaryConfig.secretaryId;
                console.log(`📋 Tentando buscar secretária com ID da config: ${secretaryId}`);
                ...
                try {
                    ...
                    if (secretarySnap.exists()) {
                        ...
                        if (secretaryData.active && secretaryData.doctorId === doctorId) {
                            ...
                            console.log(`✅ Secretária encontrada via config: ${secretaryData.name}`);
                        }
                    }
                } catch (error) {
                    console.warn(`⚠️ Erro ao buscar secretária por ID da config: ${error.message}`);
                }
            }

            // 2. Se não encontrou, buscar diretamente na collection de secretárias
            if (!secretaryInfo) {
                console.log('🔄 Buscando secretária via query na collection...');
                try {
                    const secretariesRef = collection(this.firestore, "secretaries");
                    const q = query(
                        secretariesRef,
                    ...
                    if (!querySnapshot.empty) {
                        ...
                        console.log(`✅ Secretária encontrada via query: ${secretaryData.name}`);

                        // ✅ CORRIGIR CONFIGURAÇÃO DO MÉDICO SE NECESSÁRIO
                        if (!doctorData.secretaryConfig?.secretaryId ||
                            doctorData.secretaryConfig.secretaryId !== secretaryDoc.id) {
                            console.log('🔧 Corrigindo configuração do médico...');
                            await this.fixDoctorSecretaryConfig(doctorId, secretaryDoc.id, secretaryData);
                        }
                    }
                } catch (error) {
            ...
            // 3. Se ainda não encontrou, verificar no array de secretárias
            if (!secretaryInfo && doctorData.secretaryConfig?.secretaries) {
                console.log('🔄 Buscando na lista de secretárias do médico...');
                const secretariesArray = doctorData.secretaryConfig.secretaries;
                const activeSecretary = secretariesArray.find(s => s.active === true);

                if (activeSecretary && activeSecretary.id) {
                    try {
                        const secretaryRef = doc(this.firestore, "secretaries", activeSecretary.id);
                        const secretarySnap = await getDoc(secretaryRef);

                        if (secretarySnap.exists()) {
                            const secretaryData = secretarySnap.data();
                            secretaryInfo = {
                                id: activeSecretary.id,
                                name: secretaryData.name,
                                email: secretaryData.email,
                                active: secretaryData.active,
                                permissions: secretaryData.permissions,
                            ...
                            console.log(`✅ Secretária encontrada via array: ${secretaryData.name}`);
                        }
                    } catch (error) {
                        console.warn(`⚠️ Erro ao buscar secretária do array: ${error.message}`);
                    }
                }
            }

            if (secretaryInfo) {
                console.log(`✅ Informações da secretária obtidas: ${secretaryInfo.name}`);
                return secretaryInfo;
            } else {
                console.log('📝 Nenhuma secretária ativa encontrada para este médico');
                return null;
            }
    ...
    async fixDoctorSecretaryConfig(doctorId, secretaryId, secretaryData = null) {
        try {
            console.log(`🔧 Corrigindo configuração do médico ${doctorId} com secretária ${secretaryId}`);

            let secretary = secretaryData;

            // Se não passou os dados da secretária, buscar
            if (!secretary) {
                const secretaryDoc = await getDoc(doc(this.firestore, "secretaries", secretaryId));
                if (!secretaryDoc.exists()) {
                    console.error('❌ Secretária não encontrada para correção');
                    return false;
                }
                secretary = secretaryDoc.data();
            }
            ...
            const updatedConfig = {
                secretaryConfig: {
                    ...
                    secretaryCount: 1,
                    lastSecretaryCreated: secretary.createdAt || new Date(),
                    version: "1.3", // Incrementar versão
                    lastFixed: new Date(),
                    secretaries: arrayUnion({
                        id: secretaryId,
                        name: secretary.name,
                        email: secretary.email,
                        createdAt: secretary.createdAt || new Date(),
                        active: true
                    })
                },
                updatedAt: new Date()
            ...
            await updateDoc(doctorRef, updatedConfig);
            console.log(`✅ Configuração do médico corrigida com sucesso`);
            return true;
        } catch (error) {
            console.error("❌ Erro ao corrigir configuração:", error);
            return false;
        }
    }
    ...
    /**
     * Enviar email de boas-vindas para secretária
     */
    async sendSecretaryWelcomeEmail(secretaryEmail, secretaryName, doctorName) {
        try {
            const response = await fetch('/api/secretary-welcome-email', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            console.log('✅ Email de boas-vindas enviado para secretária');
            return true;
        } catch (error) {
            console.error("❌ Erro ao enviar email de boas-vindas:", error);
            // Não fazer throw - email é não crítico
            return false;
        }
    }
