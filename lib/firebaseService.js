...
class FirebaseService {
    ...
    // Função para obter dados do clima do usuário
    async getUserWeatherData(uid) {
        try {
            ...
            if (userDoc.exists()) {
                ...
                // Verificar se o usuário tem dados de clima
                if (userData.weatherData) {
                    return {
                        weatherData: userData.weatherData,
                        currentCity: userData.address && userData.address.city
            ...
            // Retorna objeto com dados nulos e cidade padrão se não existir dados
            return {
                weatherData: null,
                currentCity: userDoc.exists() && userDoc.data().address && userDoc.data().address.city
                    ? userDoc.data().address.city
                    : "São Paulo,BR"
            };
        } catch (error) {
            console.error("Erro ao buscar dados do clima do usuário:", error);
            throw error;
        }
    }

// Função para atualizar os dados do clima do usuário
    async updateUserWeatherData(uid, weatherData, cityRequested) {
        try {
            const userRef = doc(this.firestore, "users", uid);

            // Adiciona timestamp e cidade solicitada à atualização
            const weatherDataWithMetadata = {
                ...weatherData,
                timestamp: new Date(),
                cityRequested: cityRequested // Armazenar a cidade que foi usada na requisição
            };
            ...
            console.error("Erro ao atualizar dados do clima do usuário:", error);
            throw error;
        }
    }

// Função para verificar se os dados do clima precisam ser atualizados
// Esta função agora recebe os dados já obtidos do Firestore
    async shouldUpdateWeatherData(weatherData, currentCity) {
        try {
            // Se não houver dados, deve atualizar
            if (!weatherData) {
                return true;
            }

            // Se a cidade for diferente da que foi usada na última requisição, deve atualizar
            if (weatherData.cityRequested !== currentCity) {
                return true;
            }

            // Se os dados forem mais antigos que 1 hora, deve atualizar
            ...
            console.error("Erro ao verificar dados do clima:", error);
            return true; // Em caso de erro, é melhor atualizar
        }
    }

    async getPatientStatusHistory(doctorId, patientId) {
        try {
            // Verificar se a coleção statusHistory existe
            ...
            // Buscar todos os registros de histórico de status ordenados por data (decrescente)
            const q = query(statusHistoryRef, orderBy("timestamp", "desc"));
            const querySnapshot = await getDocs(q);

            // Se não houver histórico, retornar array vazio
            if (querySnapshot.empty) {
                // Como fallback, tente buscar o statusList atual do paciente
                const patientRef = doc(this.firestore, "users", doctorId, "patients", patientId);
                const patientDoc = await getDoc(patientRef);

                if (patientDoc.exists() && patientDoc.data().statusList) {
                    // Se existe um statusList mas não um histórico, crie um histórico básico
                    return [{
                        status: patientDoc.data().statusList[0] || 'pendente',
                        timestamp: patientDoc.data().updatedAt || new Date(),
                        updatedBy: 'Sistema'
                    }];
                }
            ...
            // Transformar dados do Firestore em objetos JS
            const history = [];
