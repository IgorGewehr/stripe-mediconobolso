...
class FirebaseService {
    ...
    /**
     * Criar conta de secretária para um médico
     * @param {string} doctorId - ID do médico
     * @param {Object} secretaryData - Dados da secretária
     * @returns {Promise<Object>} - Resultado da operação
     */
    async createSecretaryAccount(doctorId, secretaryData) {
        try {
            console.log(`🔄 Criando conta de secretária para médico: ${doctorId}`);
            ...
            // 2. Verificar se médico existe e não tem secretária ativa
            ...
            if (doctorData.secretaryConfig?.hasSecretary) {
                throw new Error("Médico já possui uma secretária ativa");
            }

            // 3. Validar email único
            const existingSecretary = await this.checkSecretaryEmailExists(secretaryData.email);
            if (existingSecretary) {
                throw new Error("Este email já está sendo usado por outra secretária");
            }
            ...
            // 5. Preparar dados da secretária com permissões padrão
            ...
            // 6. Criar documento na collection secretaries
            ...
            // 7. Atualizar configuração do médico
            ...
            secretaryPermissionsCache.delete(secretaryId);
            ...
            return {
                ...
                data: {
                    ...
                    email: secretaryData.email,
                    permissions: secretaryDocData.permissions
                }
            };
            ...
            if (error.code !== 'auth/email-already-in-use') {
                try {
                    // Tentar deletar conta Auth se foi criada
                    if (error.secretaryId) {
    ...
    async getUserUnifiedContext(userId) {
        try {
            ...
            cleanExpiredCache();
            const cached = secretaryPermissionsCache.get(userId);
            if (cached && (Date.now() - cached.timestamp < CACHE_DURATION)) {
                console.log(`⚡ Contexto obtido do cache para: ${userId}`);
                return cached.data;
            }
            ...
            try {
                ...
                if (secretarySnap.exists()) {
                    ...
                    // Cache do resultado
                    secretaryPermissionsCache.set(userId, {
                        data: context,
                        timestamp: Date.now()
                    });
    ...
    async getDoctorSecretaryInfo(doctorId) {
        try {
            ...
            const secretaryId = doctorData.secretaryConfig.secretaryId;
            const secretaryRef = doc(this.firestore, "secretaries", secretaryId);
            const secretarySnap = await getDoc(secretaryRef);
