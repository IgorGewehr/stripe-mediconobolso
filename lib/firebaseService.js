...
class FirebaseService {
    ...
    async filterPatients(doctorId, filters = {}) {
        try {
            ...
            // Campos exatos que podem ser filtrados diretamente no Firestore
            const exactFilters = ['gender'];
            exactFilters.forEach(field => {
                if (filters[field]) {
                    queryRef = query(queryRef, where(field, "==", filters[field]));
                }
            });
            ...
            // Podem ser adicionados filtros adicionais aqui se necessário
