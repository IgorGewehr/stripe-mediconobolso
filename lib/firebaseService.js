...
// ====================================================
    _formatDateTimeToString(dateValue) {
        ...
        if (dateValue instanceof Date) {
            ...
            // Verifica se a string já está no formato ISO ou nosso formato personalizado
            if (dateValue.includes('T') || dateValue.includes('/')) {
                return dateValue; // Já está em formato string, retorna como está
            }
        ...
        // Formata como ISO string localizada "YYYY-MM-DDTHH:MM:SS"
        // Isso preserva data e hora exatas
        return date.toISOString();
    }

    _parseStringToDate(stringValue) {
        // Se for null, undefined ou já é um Date
        if (!stringValue || stringValue instanceof Date) return stringValue;
        ...
        // Para qualquer outro caso, retorna o valor ou um novo Date
        return stringValue || new Date();
    }

// Processa um documento inteiro, convertendo datas de string para Date
    _processConsultationDates(consultation) {
        if (!consultation) return consultation;

        const processed = {...consultation};

        // Converte consultationDate de string para Date
        if (processed.consultationDate) {
            processed.consultationDate = this._parseStringToDate(processed.consultationDate);
        }

        // Converte outras datas se necessário
        if (processed.createdAt) {
            processed.createdAt = this._parseStringToDate(processed.createdAt);
        }

        if (processed.updatedAt) {
            processed.updatedAt = this._parseStringToDate(processed.updatedAt);
        }
    ...
    async listAllConsultations(doctorId, options = {}) {
        try {
            ...
            if (options.dateFrom) {
                const fromString = this._formatDateTimeToString(options.dateFrom);
                q = query(q, where("consultationDate", ">=", fromString));
            }
            if (options.dateTo) {
                const toString = this._formatDateTimeToString(options.dateTo);
                q = query(q, where("consultationDate", "<=", toString));
            }
    ...
    async createConsultation(doctorId, patientId, consultation) {
        try {
            ...
            if (consultationData.consultationDate) {
                consultationData.consultationDate = this._formatDateTimeToString(consultationData.consultationDate);
            }

            const newConsultation = {
                ...consultationData,
                id: consultationRef.id,
                createdAt: this._formatDateTimeToString(new Date()),
                doctorId: doctorId
            };
            ...
            await updateDoc(patientRef, {
                lastConsultationDate: consultationData.consultationDate
            });

            // Retornamos o ID e convertemos de volta para Date
    ...
    async updateConsultation(doctorId, patientId, consultationId, consultation) {
        try {
            ...
            const updatedData = {
                ...consultationData,
                updatedAt: this._formatDateTimeToString(new Date())
            };
    ...
    // Adicione estas funções à classe FirebaseService
...
// Função auxiliar para formatar o tamanho do arquivo
    ...
    // ====================================================
    // Adicione estas funções ao seu FirebaseService para gerenciar medicamentos
...
// Função modificada para buscar receitas com mais detalhes
