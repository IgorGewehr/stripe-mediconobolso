...
class FirebaseService {
    ...
    async getMessagesStats() {
        try {
            console.log('🔄 Calculando estatísticas de mensagens...');

            const allMessages = await this.getAllUsersMessages();

            if (!Array.isArray(allMessages)) {
                console.warn('⚠️ getAllUsersMessages não retornou um array:', allMessages);
                return null;
            }
            ...
            const stats = {
                ...
                byType: {
                    support: allMessages.filter(m => m.type === 'support').length,
                    feedback: allMessages.filter(m => m.type === 'feedback').length,
                    bug: allMessages.filter(m => m.type === 'bug').length,
                    system: allMessages.filter(m => m.type === 'system').length
                }
            };
    ...
    /**
     * CORREÇÃO: Buscar todas as mensagens de todos os usuários (para admin) - Versão melhorada
     */
    async getAllUsersMessages(filters = {}) {
        try {
            ...
            for (const userDoc of usersSnapshot.docs) {
                ...
                try {
                    ...
                    messagesSnapshot.forEach(messageDoc => {
                        ...
                        // Validar dados da mensagem antes de adicionar
                    ...
                    // Continua para o próximo usuário em vez de falhar completamente
                }
            }
            ...
            // Retornar array vazio em vez de null para evitar erros na UI
            return [];
        }
    }

    /**
     * CORREÇÃO: Criar mensagem de usuário - Versão melhorada
     */
    async createUserMessage(userId, messageData) {
        try {
            if (!userId) {
                throw new Error('userId é obrigatório');
            }

            if (!messageData || !messageData.subject || !messageData.content) {
                throw new Error('Dados da mensagem são obrigatórios (subject e content)');
            }

            console.log(`🔄 Criando mensagem para usuário ${userId}...`);

            const messageRef = doc(collection(this.firestore, "users", userId, "messages"));
    ...
    /**
     * CORREÇÃO: Adicionar resposta a mensagem - Versão melhorada
     */
    async addMessageResponse(userId, messageId, responseData) {
        try {
            if (!userId || !messageId || !responseData) {
                throw new Error('userId, messageId e responseData são obrigatórios');
            }

            console.log(`🔄 Adicionando resposta à mensagem ${messageId}...`);

            const messageRef = doc(this.firestore, "users", userId, "messages", messageId);
            ...
            const updateData = {
                ...
                status: responseData.isAdmin ? 'in_progress' : messageData.status
            };

            // Se é resposta de admin, marcar como não lida para o usuário
            if (responseData.isAdmin) {
                updateData.hasUnreadResponses = true;
            }

            await updateDoc(messageRef, updateData);
            console.log('✅ Resposta adicionada à mensagem:', messageId);
    ...
    /**
     * CORREÇÃO: Atualizar status de uma mensagem - Versão melhorada
     */
    async updateMessageStatus(userId, messageId, status) {
        try {
            if (!userId || !messageId || !status) {
                throw new Error('userId, messageId e status são obrigatórios');
            }

            console.log(`🔄 Atualizando status da mensagem ${messageId} para ${status}...`);
            ...
            console.log('✅ Status da mensagem atualizado:', status);
            return true;
        } catch (error) {
    ...
    /**
     * CORREÇÃO: Buscar mensagens do usuário - Versão melhorada
     */
    async getUserMessages(userId) {
        try {
            ...
            console.log(`🔄 Buscando mensagens do usuário ${userId}...`);

            const messagesRef = collection(this.firestore, "users", userId, "messages");
            const q = query(messagesRef, orderBy("updatedAt", "desc"));
            ...
            console.log(`✅ ${messages.length} mensagens encontradas para o usuário`);
            return messages;
        } catch (error) {
    ...
    /**
     * CORREÇÃO: Marcar mensagens como lidas - Versão melhorada
     */
    async markMessagesAsRead(userId, messageIds = []) {
        try {
            if (!userId || !Array.isArray(messageIds)) {
                throw new Error('userId deve ser válido e messageIds deve ser um array');
            }

            if (messageIds.length === 0) {
                console.log('⚠️ Nenhuma mensagem para marcar como lida');
                return true;
            }

            console.log(`🔄 Marcando ${messageIds.length} mensagens como lidas...`);
    ...
    // ====================================================
// CORREÇÕES PARA AS FUNÇÕES DE REPORTS - ADICIONAR AO FIREBASESERVICE
// ====================================================
    ...
    async getAdminUserConversation(userId) {
        try {
            ...
            // Filtra conversas deste usuário específico
    ...
    /**
     * Criar nova conversa admin->usuário
     * @param {string} userId - ID do usuário
     * @param {string} initialMessage - Mensagem inicial
     * @param {Object} adminInfo - Informações do admin
     * @returns {string} - ID da conversa criada
     */
    ...
    /**
     * Enviar mensagem admin na conversa
    ...
    /**
     * Buscar estatísticas de chats admin
     * @returns {Object} - Estatísticas dos chats admin
     */
    async getAdminChatStats() {
        try {
            console.log('📊 Calculando estatísticas de chats admin...');

            const conversations = await this.getAllAdminUserConversations();

            const stats = {
                total: conversations.length,
                active: conversations.filter(c => c.status !== 'resolved').length,
                resolved: conversations.filter(c => c.status === 'resolved').length,
                unreadByUsers: conversations.filter(c => c.hasUnreadResponses === true).length,
                unreadByAdmins: conversations.filter(c => c.hasUnreadFromUser === true).length,
            ...
            console.log('✅ Estatísticas de chats admin calculadas:', stats);
            return stats;
        } catch (error) {
            console.error("❌ Erro ao calcular estatísticas de chats admin:", error);
            return {
                total: 0,
                active: 0,
                resolved: 0,
    ...
    /**
     * Verificar se usuário tem conversas admin não lidas
     * @param {string} userId - ID do usuário
     * @returns {boolean} - True se tem mensagens não lidas
     */
    async userHasUnreadAdminMessages(userId) {
        try {
            const conversation = await this.getAdminUserConversation(userId);
            return conversation?.hasUnreadResponses === true;
        } catch (error) {
            console.error("❌ Erro ao verificar mensagens admin não lidas:", error);
            return false;
        }
    }
    /**
     * Marcar conversa admin como lida pelo usuário
     * @param {string} userId - ID do usuário
     * @returns {boolean} - Sucesso da operação
     */
    async markAdminConversationAsReadByUser(userId) {
        try {
            const conversation = await this.getAdminUserConversation(userId);
            if (conversation?.id) {
                return await this.markReportAsReadByUser(conversation.id);
            }
            return true;
        } catch (error) {
            console.error("❌ Erro ao marcar conversa admin como lida:", error);
            return false;
        }
    }
    ...
    async createReport(userId, reportData) {
        try {
            ...
            const newReport = {
                id: reportRef.id,
                userId: userId, // GARANTIR QUE O USERID ESTÁ SENDO SALVO
                userName: userData.fullName || userData.email || 'Usuário Anônimo',
                ...
                updatedAt: new Date(),
                responses: [],
                hasUnreadResponses: false, // Para o usuário
                hasUnreadFromUser: true,   // Para o admin
                lastResponseAt: null
            };
    ...
    async getUserUnreadReportsCount(userId) {
        try {
            if (!userId) {
                return 0;
            }

            console.log(`🔄 Contando reports não lidos para usuário ${userId}...`);

            const reportsRef = collection(this.firestore, "reports");
            const q = query(
                reportsRef,
                where("userId", "==", userId),
                where("hasUnreadResponses", "==", true)
            );

            const querySnapshot = await getDocs(q);
            const count = querySnapshot.size;

            console.log(`✅ ${count} reports não lidos encontrados para o usuário ${userId}`);
            return count;
        } catch (error) {
            console.error("❌ Erro ao contar reports não lidos:", error);
            return 0;
        }
    }

// ✨ FUNÇÃO MELHORADA PARA ADICIONAR RESPOSTA
    async addReportResponse(reportId, responseData) {
        try {
            ...
            // ✨ LÓGICA CORRIGIDA PARA FLAGS DE LEITURA
            if (responseData.isAdmin) {
                // Admin respondeu - marcar como não lida para o usuário
                updateData.status = 'in_progress';
                updateData.hasUnreadResponses = true;  // ✨ USUÁRIO TEM RESPOSTA NÃO LIDA
                updateData.hasUnreadFromUser = false;  // Admin já viu a mensagem do usuário
            } else {
                // Usuário respondeu - marcar como não lida para o admin
                updateData.hasUnreadFromUser = true;   // Admin tem nova mensagem do usuário
                updateData.hasUnreadResponses = false; // Usuário viu sua própria resposta
            }
...
// ====================================================
// BUSCAR TODOS OS REPORTS (PARA ADMINS) - OTIMIZADO
// ====================================================
...
// ====================================================
// BUSCAR REPORTS DO USUÁRIO - OTIMIZADO
// ====================================================

// ====================================================
// ADICIONAR RESPOSTA A UM REPORT
// ====================================================
...
// ====================================================
    async updateReportStatus(reportId, status, updatedBy = null) {
        try {
            ...
            // Se está sendo resolvido, limpar flags de não lidas
            if (status === 'resolved') {
...
// ====================================================
// BUSCAR ESTATÍSTICAS DOS REPORTS - OTIMIZADO
// ====================================================
    async getReportsStats() {
        try {
            console.log('🔄 Calculando estatísticas dos reports...');

            // Buscar todos os reports de uma vez (muito mais rápido que iterar usuários)
...
// ====================================================
    async deleteReport(reportId) {
        try {
            ...
            console.log(`🔄 Deletando report ${reportId}...`);
...
// ====================================================
// MIGRAÇÃO DOS DADOS ANTIGOS (EXECUTAR UMA VEZ)
// ====================================================
    async migrateOldMessagesToReports() {
        try {
            console.log('🔄 Iniciando migração de mensagens antigas para reports...');

            // Buscar todas as mensagens antigas
            const oldMessages = await this.getAllUsersMessages();
            let migratedCount = 0;

            for (const oldMessage of oldMessages) {
                try {
                    // Criar novo report com dados da mensagem antiga
                    const reportRef = doc(collection(this.firestore, "reports"));

                    const newReport = {
                        id: reportRef.id,
                        userId: oldMessage.userId,
                        userName: oldMessage.userName || 'Usuário Anônimo',
                        userEmail: oldMessage.userEmail || 'Email não informado',
                        subject: oldMessage.subject || 'Mensagem migrada',
                        content: oldMessage.content || '',
                        type: oldMessage.type || 'support',
                        status: oldMessage.status || 'new',
                        priority: oldMessage.priority || 'medium',
                        createdAt: oldMessage.createdAt || new Date(),
                        updatedAt: oldMessage.updatedAt || new Date(),
                        responses: oldMessage.responses || [],
                        hasUnreadResponses: oldMessage.hasUnreadResponses || false,
                        hasUnreadFromUser: oldMessage.status === 'new',
                        lastResponseAt: oldMessage.lastResponseAt || null,
                        // Marcar como migrado
                        migrated: true,
                    ...
                    console.log(`✅ Migrado: ${oldMessage.subject}`);
                } catch (error) {
                    console.warn(`⚠️ Erro ao migrar mensagem ${oldMessage.id}:`, error);
                }
            }

            console.log(`✅ Migração concluída: ${migratedCount} mensagens migradas`);
