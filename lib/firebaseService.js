...
// ==============================================
    ...
    /**
     * ✅ FUNÇÃO PRINCIPAL REFATORADA - SEM LOGOUT DO MÉDICO
     */
    async createSecretaryAccount(doctorId, secretaryData) {
        try {
            console.log(`🚀 Iniciando criação de secretária (versão sem logout)...`);

            // STEP 1: Validações
            const validation = await this.validateSecretaryCreation(doctorId, secretaryData);
            const { doctorData, secretaryCount } = validation;

            // STEP 2: Usar Cloud Function para criar secretária sem afetar sessão atual
            const result = await this.createSecretaryViaCloudFunction(doctorId, secretaryData);

            if (result.success) {
                // STEP 3: Atualizar configuração do médico
                await this.updateDoctorConfiguration(
                    doctorId,
                    result.secretaryId,
                    secretaryData,
                    secretaryCount
                ...
                // STEP 4: Verificar criação
                await this.verifySecretaryCreation(doctorId, result.secretaryId);

                console.log(`✅ Secretária criada com sucesso! ID: ${result.secretaryId}`);

                return {
                    success: true,
                    secretaryId: result.secretaryId,
                    needsDoctorRelogin: false, // ✅ NÃO PRECISA MAIS DE RE-LOGIN
                    data: {
            ...
            throw new Error(result.error || 'Falha na criação da secretária');
    ...
    /**
     * ✅ NOVA FUNÇÃO - CRIAR SECRETÁRIA VIA CLOUD FUNCTION
     * Esta função evita o problema de logout automático
     */
    async createSecretaryViaCloudFunction(doctorId, secretaryData) {
        try {
            // Opção 1: Se você tem Cloud Functions configuradas
            ...
            if (!response.ok) {
                throw new Error('Erro na Cloud Function');
            }

            return await response.json();

        } catch (error) {
            console.warn('⚠️ Cloud Function não disponível, usando método alternativo...');

            // Opção 2: Método alternativo - criar sem fazer login
            return await this.createSecretaryAlternativeMethod(doctorId, secretaryData);
        }
    }

    /**
     * ✅ MÉTODO ALTERNATIVO - CRIAR DOCUMENTOS SEM AUTH
     */
    async createSecretaryAlternativeMethod(doctorId, secretaryData) {
        try {
            console.log('🔄 Usando método alternativo para criar secretária...');

            // Gerar ID único para secretária
            const secretaryId = `secretary_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

            // Criar documento da secretária diretamente no Firestore
            const secretaryDocData = {
                ...
                createdAt: new Date(),
                createdBy: doctorId,
                // ✅ MARCAR COMO PENDENTE DE ATIVAÇÃO
                needsActivation: true,
                activationToken: this.generateActivationToken(),
                tempPassword: secretaryData.password, // Armazenar temporariamente
                lastLogin: null,
                loginCount: 0,
                version: "2.0"
            };
    ...
    /**
     * ✅ ATIVAÇÃO DA CONTA DA SECRETÁRIA
     * Esta função será chamada quando a secretária fizer primeiro login
     */
    async activateSecretaryAccount(activationToken, newPassword) {
        try {
            console.log('🔓 Ativando conta da secretária...');

            // Buscar secretária pelo token
            ...
            const secretaryId = secretaryDoc.id;
            ...
            // Atualizar documento da secretária
            await updateDoc(secretaryDoc.ref, {
            ...
            console.log('✅ Conta da secretária ativada com sucesso');
    ...
    /**
     * ✅ BUSCAR INFORMAÇÕES DA SECRETÁRIA - MELHORADA
     */
    async getDoctorSecretaryInfo(doctorId) {
        try {
            ...
            // Buscar diretamente na collection de secretárias
            const secretariesRef = collection(this.firestore, "secretaries");
            const q = query(
                secretariesRef,
    ...
    /**
     * ✅ VERIFICAR SE EMAIL EXISTE - MELHORADA
     */
    async checkEmailExistsInSystem(email) {
        try {
            // Verificar em usuários (médicos)
            const usersRef = collection(this.firestore, "users");
            ...
            // Verificar em secretárias (incluindo pendentes de ativação)
            const secretariesRef = collection(this.firestore, "secretaries");
            ...
            return { exists: true }; // Seguro assumir que existe em caso de erro
        }
    }
     ...
     */
    async sendSecretaryActivationEmail(email, name, activationToken) {
        try {
            const activationLink = `${process.env.NEXT_PUBLIC_APP_URL || 'https://mediconobolso.app'}/activate-secretary?token=${activationToken}`;

            const response = await fetch('/api/secretary-activation-email', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
