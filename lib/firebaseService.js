    ...
    async listConsultations(doctorId, patientId) {
        try {
            ...
            if (!doctorId || !patientId) {
                console.error("doctorId ou patientId inválidos:", { doctorId, patientId });
                return [];
            }

            console.log(`Buscando consultas para o paciente ${patientId}`);

            // Garantir que estamos usando o caminho correto e filtros
            const q = query(
            ...
            querySnapshot.forEach(docSnap => {
                // Garantir que cada consulta tenha o patientId
                const consultation = docSnap.data();
                consultations.push({
                    id: docSnap.id,
                    ...consultation,
                    patientId: patientId  // Garantir que o patientId esteja incluído
                });
            });

            console.log(`Recuperadas ${consultations.length} consultas para o paciente ${patientId}`);
            return consultations;
        } catch (error) {
            console.error(`Erro ao listar consultas para o paciente ${patientId}:`, error);
            return [];
        }
    }
