...
export async function sendWelcomeEmail(userEmail, userName, appLink) {
    console.log(`📧 Iniciando envio de email de boas-vindas para: ${userEmail}`);
...
export async function sendHelpVideoEmail(userEmail, userName, appLink) {
    console.log(`📧 Iniciando envio de email de ajuda para: ${userEmail}`);
    ...
    try {
        ...
        <body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f5f5f5; color: #333333;">
            <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden;">
                ...
                <div style="padding: 40px 30px;">
                    <h1 style="color: #151B3B; font-size: 24px; margin: 0 0 20px 0; text-align: center;">Precisa de ajuda para acessar?</h1>
      ...
      `
        ...
        console.log(`📧 Enviando email de ajuda com vídeo...`);
        const info = await transporter.sendMail(mailOptions);
        console.log('✅ Email de ajuda enviado com sucesso:', info.messageId);
...
// Função de teste: Email de ajuda super simples (para debug)
export async function sendSimpleHelpEmail(userEmail, userName, appLink) {
    console.log(`📧 [TESTE] Enviando email de ajuda SIMPLES para: ${userEmail}`);
    ...
    try {
        ...
        const mailOptions = {
            from: process.env.EMAIL_FROM,
            to: userEmail,
            subject: 'Teste - Não consegui acessar o mediconobolso',
            html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9f9f9;">
                <div style="background-color: white; padding: 30px; border-radius: 8px;">
                    <h1 style="color: #151B3B;">Ajuda para Acessar</h1>
                    <p>Olá Dr. ${userName || 'Médico'},</p>
                    <p>Este é um email de ajuda para acessar o Médico no Bolso.</p>
                    
                    <div style="background-color: #e3f2fd; padding: 15px; border-radius: 5px; margin: 20px 0;">
                        <h3>Como Acessar:</h3>
                        <ol>
                            <li>Clique no botão abaixo</li>
                            <li>Use seu email: <strong>${userEmail}</strong></li>
                            <li>Digite sua senha</li>
                        </ol>
                    </div>
                    
                    <div style="text-align: center; margin: 30px 0;">
                        <a href="${appLink}" style="background-color: #F9B934; color: black; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-weight: bold;">
                            ACESSAR PLATAFORMA
                        </a>
                    </div>
                    
                    <p>Dúvidas? Responda este email.</p>
                    <p>Equipe Médico no Bolso</p>
                </div>
            </div>
            `
        };

        console.log(`📧 [TESTE] Enviando email super simples...`);
        const info = await transporter.sendMail(mailOptions);
        console.log('✅ [TESTE] Email super simples enviado:', info.messageId);

        return { success: true, messageId: info.messageId, type: 'super-simple-test' };
    } catch (error) {
        console.error('❌ [TESTE] Erro no email super simples:', error);
        return { success: false, error: error.message, type: 'super-simple-test' };
    }
}
...
export async function sendBothWelcomeEmails(userEmail, userName, appLink) {
    ...
    try {
        ...
        console.log('📧 1/2 - Enviando email de boas-vindas...');
        try {
            results.welcome = await sendWelcomeEmail(userEmail, userName, appLink);
            console.log(`✅ Email de boas-vindas: ${results.welcome.success ? 'SUCESSO' : 'FALHA'}`);
        } catch (welcomeError) {
            console.error('❌ Erro no email de boas-vindas:', welcomeError);
            results.welcome = { success: false, error: welcomeError.message, messageId: null };
        }

        // Aguardar entre envios (menor tempo)
        console.log('⏳ Aguardando 2 segundos entre emails...');
        await new Promise(resolve => setTimeout(resolve, 2000));

        // SEGUNDO EMAIL: Ajuda
        console.log('📧 2/2 - Enviando email de ajuda com vídeo...');
        try {
            results.help = await sendHelpVideoEmail(userEmail, userName, appLink);
            console.log(`✅ Email de ajuda: ${results.help.success ? 'SUCESSO' : 'FALHA'}`);
        } catch (helpError) {
            console.error('❌ Erro no email de ajuda:', helpError);
        ...
        console.log('📊 Resultado final dos emails:', response.message);
        return response;

    } catch (error) {
        console.error('❌ Erro geral ao enviar ambos os emails:', error);
        return {
            success: false,
            error: error.message,
