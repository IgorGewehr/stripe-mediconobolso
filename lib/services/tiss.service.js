...
async function fetchApi(endpoint, options = {}) {
  ...
  console.log(`[TISS API] >>> Request: ${options.method || 'GET'} ${API_BASE}${endpoint}`);

  try {
    const token = await getAuthToken();
    console.log('[TISS API] Token obtido com sucesso');
    ...
    console.log(`[TISS API] <<< Response: ${response.status} (${elapsed}ms)`);
    ...
    try {
      ...
      console.warn('[TISS API] Resposta não é JSON válido:', text?.substring(0, 100));
      data = {};
    }

    if (!response.ok) {
      console.error(`[TISS API] Erro HTTP ${response.status}:`, data);
      throw new Error(data.error || `HTTP error! status: ${response.status}`);
    }

    console.log(`[TISS API] Dados recebidos:`, {
      endpoint,
    ...
    const elapsed = Date.now() - startTime;
    console.error(`[TISS API] !!! Erro em ${endpoint} (${elapsed}ms):`, error.message);
    throw error;
  }
}
