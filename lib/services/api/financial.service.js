...
export async function createContaPagar(data) {
  ...
  try {
    ...
    const payload = {
      descricao: data.descricao,
      valor_original: data.valor, // Backend expects 'valor_original', not 'valor'
      data_vencimento: data.dataVencimento,
      tipo_despesa: data.tipoDespesa, // Required by backend
    };

    // Optional fields
    if (data.fornecedorId) payload.fornecedor_id = data.fornecedorId;
    if (data.fornecedorNome) payload.fornecedor_nome = data.fornecedorNome;
    if (data.dataCompetencia) payload.data_competencia = data.dataCompetencia;
    if (data.recorrencia) payload.recorrencia = data.recorrencia;
    if (data.totalParcelas) payload.total_parcelas = data.totalParcelas;
    if (data.categoriaId) payload.categoria_id = data.categoriaId;
    if (data.centroCustoId) payload.centro_custo_id = data.centroCustoId;
    if (data.observacoes) payload.observacoes = data.observacoes;
...
export async function listMovimentacoesBancarias(options = {}) {
  ...
  if (options.contaBancariaId) params.conta_bancaria_id = options.contaBancariaId;
  if (options.tipo) params.tipo = options.tipo;
  if (options.dataInicio) params.data_inicio = options.dataInicio;
  if (options.dataFim) params.data_fim = options.dataFim;
  if (options.page) params.page = options.page;
