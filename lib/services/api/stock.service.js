...
function normalizeStock(item) {
  ...
  return {
    id: item.id,
    itemName: item.nome_item,
    category: item.categoria,
    currentQuantity: item.quantidade_atual,
    lastMovementType: item.tipo_ultima_movimentacao,
...
function denormalizeStock(data) {
  return {
    nome_item: emptyToNull(data.itemName),
    categoria: emptyToNull(data.category),
    
    quantidade_atual: data.currentQuantity !== undefined ? Number(data.currentQuantity) : undefined,
    observacoes: emptyToNull(data.notes)
  };
}
