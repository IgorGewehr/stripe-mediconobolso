...
export async function startOAuth(tenantId) {
  const response = await fetch(`${API_BASE_URL}/facebook/oauth/start?tenantId=${tenantId}`);
...
export async function connectPage({ pageId, pageAccessToken, pageName, pageCategory, aiEnabled = false }) {
  ...
  const response = await fetch(`${API_BASE_URL}/facebook/auth`, {
    method: 'POST',
    headers,
    body: JSON.stringify({
      pageId,
...
export async function getStatus() {
  ...
  if (!response.ok) {
    // If Facebook is disabled on backend, return disconnected status
...
export async function toggleAI(enabled) {
  ...
  const response = await fetch(`${API_BASE_URL}/facebook/ai`, {
    method: 'POST',
    headers,
...
export async function sendMessage({ recipientId, message, mediaUrl, mediaType }) {
  ...
  const response = await fetch(`${API_BASE_URL}/facebook/messages/send`, {
    method: 'POST',
    headers,
    body: JSON.stringify({
