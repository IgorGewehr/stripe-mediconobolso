 ...
 *
 * IMPORTANT: Backend expects snake_case field names, but frontend uses camelCase.
 * All data is converted before sending to backend.
 */
...
export async function updateSettings(settings) {
    ...
    const payload = toSnakeCase(settings);
    const response = await apiService.put(`${API_BASE}/current/settings`, payload);
...
/**
 * List all staff members (doctors + secretaries) unified
 * @param {Object} options
 * @param {boolean} [options.activeOnly=true] - Only active members
 * @param {'doctor'|'secretary'} [options.staffType] - Filter by type
 * @returns {Promise<import('../../types/clinic.types').StaffMember[]>}
 */
export async function listStaff({ activeOnly = true, staffType } = {}) {
    const params = { active_only: activeOnly };
    if (staffType) params.staff_type = staffType;
    return apiService.get(`${API_BASE}/staff`, params);
}
...
export async function updateDoctorPermissions(doctorId, permissions) {
    // Backend expects snake_case field names
    const payload = toSnakeCase(permissions);
    const response = await apiService.put(`${API_BASE}/doctors/${doctorId}/permissions`, payload);
    return toCamelCase(response);
}
...
export async function inviteDoctor(invite) {
    // Backend expects snake_case field names
    const payload = toSnakeCase(invite);
    const response = await apiService.post(`${API_BASE}/doctors/invite`, payload);
    return toCamelCase(response);
}
...
export async function acceptInvite(data) {
    // Backend expects snake_case: invite_token
    const payload = toSnakeCase(data);
    const response = await apiService.post(`${API_BASE}/invites/accept`, payload);
    return toCamelCase(response);
}
...
export async function createClinicSecretary(data) {
    // Backend expects snake_case: scope_type, scope_doctors, single_doctor_id
    const payload = toSnakeCase(data);
    const response = await apiService.post(`${API_BASE}/secretaries`, payload);
    return toCamelCase(response);
}
...
export async function updateClinicSecretary(secretaryId, data) {
    const payload = toSnakeCase(data);
    const response = await apiService.put(`${API_BASE}/secretaries/${secretaryId}`, payload);
    return toCamelCase(response);
}
...
export async function updateSecretaryScope(secretaryId, scope) {
    // Backend expects: scope_type, scope_doctors, single_doctor_id
    const payload = toSnakeCase(scope);
    const response = await apiService.put(`${API_BASE}/secretaries/${secretaryId}/scope`, payload);
    return toCamelCase(response);
}
...
export async function updateSecretaryPermissions(secretaryId, permissions) {
    // Backend expects snake_case: view_details, can_create_patients, etc.
    const payload = toSnakeCase(permissions);
    const response = await apiService.put(`${API_BASE}/secretaries/${secretaryId}/permissions`, payload);
    return toCamelCase(response);
}
...
export async function createRoom(data) {
    const payload = toSnakeCase(data);
    const response = await apiService.post(`${API_BASE}/rooms`, payload);
    return toCamelCase(response);
}
...
export async function updateRoom(roomId, data) {
    const payload = toSnakeCase(data);
    const response = await apiService.put(`${API_BASE}/rooms/${roomId}`, payload);
    return toCamelCase(response);
}
...
export async function sharePatient(patientId, doctorIds) {
    // Backend expects snake_case: doctor_ids
    return apiService.post(`/patients/${patientId}/share`, { doctor_ids: doctorIds });
}
...
export async function unsharePatient(patientId, doctorIds) {
    // Backend expects snake_case: doctor_ids
    return apiService.post(`/patients/${patientId}/unshare`, { doctor_ids: doctorIds });
}
