...
export async function listConvenios(params = {}) {
  const { requestId, startTime } = logger.operationStart('LIST');
  try {
    ...
    logger.operationSuccess(requestId, startTime, 'LIST');
    return response;
  } catch (error) {
    logger.operationError(requestId, startTime, 'LIST', error);
    throw error;
  }
}
...
export async function getConvenio(id) {
  const { requestId, startTime } = logger.operationStart('GET', id);
  try {
    const response = await apiService.get(`${API_BASE}/${id}`);
    logger.operationSuccess(requestId, startTime, 'GET', id);
    return response;
  } catch (error) {
    logger.operationError(requestId, startTime, 'GET', error, id);
    throw error;
  }
}
...
export async function createConvenio(data) {
  const { requestId, startTime } = logger.operationStart('CREATE', null, data);
  try {
    ...
    logger.operationSuccess(requestId, startTime, 'CREATE', null, response);
    return response;
  } catch (error) {
    logger.operationError(requestId, startTime, 'CREATE', error, null, data);
    throw error;
  }
}
...
export async function updateConvenio(id, data) {
  const { requestId, startTime } = logger.operationStart('UPDATE', id, data);
  try {
    // Convert to snake_case for API
    const payload = {};
    ...
    logger.operationSuccess(requestId, startTime, 'UPDATE', id, response);
    return response;
  } catch (error) {
    logger.operationError(requestId, startTime, 'UPDATE', error, id, data);
    throw error;
  }
}
...
export async function deleteConvenio(id) {
  const { requestId, startTime } = logger.operationStart('DELETE', id);
  try {
    await apiService.delete(`${API_BASE}/${id}`);
    logger.operationSuccess(requestId, startTime, 'DELETE', id);
  } catch (error) {
    logger.operationError(requestId, startTime, 'DELETE', error, id);
    throw error;
  }
}
