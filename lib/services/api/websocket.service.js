...
class WebSocketService {
  constructor() {
    ...
    this.tokenRefreshIntervalMs = 50 * 60 * 1000; // Check every 50 minutes
  ...
  isTokenExpiringSoon(token) {
    try {
      ...
      // Token is expiring soon if less than 5 minutes remaining
  ...
  /**
   * Obtém o token JWT do Firebase, forçando renovação se necessário
  ...
  async getAuthToken(forceRefresh = false) {
    ...
    // Get cached token first to check expiration
    if (!forceRefresh) {
      try {
        ...
        console.log('[WS] Token expiring soon, forcing refresh...');
      } catch {
        // If error getting cached token, force refresh
      }
    }

    // Force refresh the token
    console.log('[WS] Refreshing Firebase token...');
    return user.getIdToken(true);
  }

  /**
   * Conecta ao WebSocket
   * @param {boolean} forceTokenRefresh - Força renovação do token (usado em reconexões)
   * @returns {Promise<void>}
   */
  async connect(forceTokenRefresh = false) {
    ...
    try {
      // Force refresh on reconnections to ensure valid token
      const shouldForceRefresh = forceTokenRefresh || this.reconnectAttempts > 0;
      const token = await this.getAuthToken(shouldForceRefresh);
      const wsUrl = `${WS_BASE_URL}/ws?token=${encodeURIComponent(token)}`;

      console.log('[WS] Connecting to WebSocket...', shouldForceRefresh ? '(with fresh token)' : '');
  ...
  /**
   * Inicia intervalo para verificar e renovar token
   * Reconecta com token novo antes que expire
   */
    ...
    this.tokenRefreshInterval = setInterval(async () => {
      try {
        ...
        if (this.isTokenExpiringSoon(token)) {
          console.log('[WS] Token expiring soon, reconnecting with fresh token...');
          // Close current connection and reconnect with new token
          if (this.ws) {
            this.ws.close(4000, 'Token refresh');
          }
          // Will auto-reconnect via handleDisconnect with forceTokenRefresh
        }
      } catch (error) {
        console.warn('[WS] Error checking token expiration:', error);
      }
    }, this.tokenRefreshIntervalMs);
  }

  /**
   * Para o intervalo de verificação de token
   */
