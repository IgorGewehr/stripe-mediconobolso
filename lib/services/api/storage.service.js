...
const storageService = {
  ...
  async uploadProfilePhoto(file, userId) {
    const path = this.generateFilePath(userId, 'profilePhoto', file.name);
    return this.uploadFile(file, {
      path,
      category: FileCategories.PROFILE_PHOTO,
      entityType: 'User',
      entityId: userId,
    });
  },
   ...
   * @returns {Promise<string>} URL pública da foto
   */
  async uploadPatientPhoto(file, doctorId, patientId) {
    const path = `users/${doctorId}/patients/${patientId}/profilePhoto/${Date.now()}_${file.name}`;
    return this.uploadFile(file, {
      path,
      category: FileCategories.PROFILE_PHOTO,
      entityType: 'Patient',
   ...
   * @param {string} doctorId - ID do médico
   * @param {string} patientId - ID do paciente
   * @param {Object} options - Opções adicionais (category, description)
   * @returns {Promise<Object>} Dados do arquivo uploadado
   */
  async uploadPatientDocument(file, doctorId, patientId, options = {}) {
    const { category = 'Geral', description } = options;
    const path = `users/${doctorId}/patients/${patientId}/documents/${Date.now()}_${file.name}`;

    try {
      const result = await this.uploadFile(file, {
        path,
        category: FileCategories.PATIENT_DOCUMENT,
        entityType: 'Patient',
        entityId: patientId,
      });

      // Retorna objeto com URL e metadados
      return {
        url: result,
        path,
        category,
      ...
      console.error('[StorageService] Error uploading patient document:', error);
      throw error;
    }
  },
