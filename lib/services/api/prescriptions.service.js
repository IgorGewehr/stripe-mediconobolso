...
const prescriptionsService = {
  ...
  async addFavoriteMedication(medication) {
    ...
    try {
      const response = await apiService.post(`${ENDPOINT}/drugs/${medication.id}/favorite`, {
        medication_name: medication.name || medication.drugName,
        default_dosage: medication.defaultDosage || medication.dosage,
        default_quantity: medication.defaultQuantity || medication.quantity,
        concentration: medication.concentration,
        controlled_type: medication.controlledType || 'none',
      });
  ...
  async removeFavoriteMedication(medicationId) {
    const medLogger = createCrudLogger('Medicamento Favorito');
    const { requestId, startTime } = medLogger.operationStart('REMOVE_FAVORITE', medicationId);

    try {
      await apiService.delete(`${ENDPOINT}/drugs/${medicationId}/favorite`);
      medLogger.operationSuccess(requestId, startTime, 'REMOVE_FAVORITE', medicationId);
      return { success: true };
    } catch (error) {
      // Se 404, remove de localStorage
      if (error.status === 404) {
        console.log('Endpoint favoritos nao disponivel, removendo de localStorage');
        return this._removeLocalFavorite(medicationId);
      }
      const { userMessage } = medLogger.operationError(requestId, startTime, 'REMOVE_FAVORITE', error, medicationId);
      error.userMessage = userMessage;
      throw error;
    }
  },
