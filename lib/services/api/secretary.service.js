  ...
  async getDoctorSecretaryInfo(doctorId) {
    try {
      if (!doctorId || typeof doctorId !== 'string') {
        console.warn('[SecretaryService] getDoctorSecretaryInfo: doctorId inválido');
        return null;
      }

      // Buscar as secretárias do médico
      const response = await apiService.get(`${ENDPOINT}/doctor/${doctorId}`, {
        active: true,
        limit: 1,
      });
