 ...
 *
 * Usa a nova API /clinics/secretaries (substituindo a API deprecated /secretaries)
 */

import apiService from './apiService';

// Novo endpoint (recomendado)
const ENDPOINT = '/clinics/secretaries';
// Endpoint deprecated (funciona até 2026-06-01)
const DEPRECATED_ENDPOINT = '/secretaries';
...
const secretaryService = {
   ...
   * Nova API usa POST /{id}/deactivate
   */
   ...
   * Nova API usa POST /{id}/reactivate
   */
   ...
   * Usa o endpoint /clinics/summary que retorna contagens
   */
  async count() {
    const response = await apiService.get('/clinics/summary');
    return response.active_secretaries;
  },
 ...
 * Nova API /clinics/secretaries retorna campos de escopo
 */
function normalizeSecretary(secretary) {
  ...
  return {
    id: secretary.id,
    doctorId: secretary.doctor_id || secretary.single_doctor_id,
    ...
    // Escopo (nova API)
    scopeType: secretary.scope_type,
    scopeDoctors: secretary.scope_doctors || [],
    singleDoctorId: secretary.single_doctor_id,
    // Permissões
 ...
 * Nova API /clinics/secretaries usa campos de escopo
 */
function denormalizeSecretary(secretary) {
  ...
  // Escopo (nova API)
  if (secretary.scopeType !== undefined) payload.scope_type = secretary.scopeType;
  if (secretary.scopeDoctors !== undefined) payload.scope_doctors = secretary.scopeDoctors;
  if (secretary.singleDoctorId !== undefined) payload.single_doctor_id = secretary.singleDoctorId;

  // Legacy: doctor_id mapeia para single_doctor_id
  if (secretary.doctorId !== undefined && !secretary.singleDoctorId) {
    payload.single_doctor_id = secretary.doctorId;
    payload.scope_type = 'single_doctor';
  }
