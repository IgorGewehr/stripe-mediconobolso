...
class WhatsAppService {
   ...
   * @param {string} messageData.message - Message content (backend expects 'message' not 'text')
   * @param {string} [messageData.text] - DEPRECATED: Use 'message' instead
   * @param {string} [messageData.message_type] - Message type (text, image, video, audio, document)
   * @param {string} [messageData.media_url] - URL of media to send
   * @returns {Promise<Object>} Send result
   */
  async sendMessage(tenantId, messageData) {
    // Normalize the payload for backend compatibility
    const payload = {
      to: messageData.to,
      // Backend expects 'message' not 'text'
      message: messageData.message || messageData.text,
      message_type: messageData.message_type || messageData.type,
      media_url: messageData.media_url || messageData.mediaUrl,
    };

    const response = await apiService.post(`/whatsapp/messages/${tenantId}/send`, payload);
    return response;
  }
